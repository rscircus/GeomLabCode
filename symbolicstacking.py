...
def drawSolution(stacking,image):
    ...
    for i in range(0,len(stacking)):
        ...
        for k in range(0,numberOfRadi):
            ...
            y=int(tmp[0])
            r=int(tmp[2+k])
            cv2.circle(image,(x,y),r , [colorValue,colorValue,colorValue]  ,thickness=-1, lineType=8, shift=0)
            cv2.circle(image,(x,y),r ,[colorValue-19,colorValue-19,colorValue-19]  ,thickness=2, lineType=8, shift=0)
        cv2.circle(image,(int(tmp[1]),int(tmp[0])),int(tmp[2]),[0,0,0] ,thickness=2, lineType=8, shift=0)
...
def drawSolutionWeighted(stacking,image):
    ...
    for i in range(0,len(stacking)):
        ...
        for k in range(0,numberOfRadi):
            ...
            r=int(tmp[2+k])
            cv2.circle(image,(x,y),r , [colorValue,colorValue,colorValue]  ,thickness=-1, lineType=8, shift=0)
            cv2.circle(image,(x,y),r ,[colorValue-10,colorValue-10,colorValue-10]  ,thickness=1, lineType=8, shift=0)
        cv2.circle(image,(int(tmp[1]),int(tmp[0])),int(tmp[2]),[0,0,0] ,thickness=1, lineType=8, shift=0)
...
def drawing4H(stacking,image):
    ...
    for i in range(0,len(stacking)):
        ...
        if(j==1):
            cv2.circle(image,(x,y),r ,[22-19,22-19,22-19]  ,thickness=2, lineType=8, shift=0)
            bo=False
...
def drawPie(circle,pieces,angle,image):
    ...
    cv2.circle(data1,(y,x),r , [colorValue,colorValue,colorValue]  ,thickness=-1, lineType=8, shift=0)
    cv2.circle(data1,(y,x),r ,[colorValue-19,colorValue-19,colorValue-19]  ,thickness=2, lineType=8, shift=0)
    ...
    for p in cPieces:
        ...
        cv2.line(data1, (int(y), int(x)), (int(c[1]), int(c[0])), (0, 255, 0), thickness=2)
...
def drawPie2(circle,pieces,angle,image):
    ...
    img1.pieslice(shape, start =anglesClock[0]  , end=anglesClock[1], fill =color1PIL, outline ="black")

    img1.pieslice(shape, start =anglesClock[1]  , end=anglesClock[2], fill =color2PIL, outline ="black")

    img1.pieslice(shape, start =anglesClock[2]  , end=anglesClock[3], fill =color3PIL, outline ="black")
